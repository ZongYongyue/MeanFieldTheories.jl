"""
Plot HF band structure and magnon dispersion as two separate panels.

Run:
    julia --project=examples examples/draft/plot.jl
"""
...
for ni in 1:2#axes(mag_mat, 2)
    lines!(ax2, mag_arc, mag_mat[:, ni]; color = :steelblue, linewidth = 1.2,
           label = ni == 1 ? "magnon" : nothing)
end
#hlines!(ax2, 0.0; color = :red, linewidth = 0.8, linestyle = :dash, label = "E = 0")
axislegend(ax2; position = :rt, merge = true)
xlims!(ax2, mag_arc[1], mag_arc[end])

display(fig)
save("docs/src/fig/magnon_dispersion.png", fig; px_per_unit = 2)
println("Saved: docs/src/fig/magnon_dispersion.png")
